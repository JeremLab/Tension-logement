    ...
    left_join(
      data %>%
        ...
        summarise(places_res = ifelse(all(is.na(places)), 0, max(places, na.rm = TRUE)), .groups = "drop") %>%
        {
          if (!avec_residence) {
            group_by(., across(all_of(c(groupe_vars, "num_tour")))) %>%
              summarise(places_res = sum(places_res, na.rm = TRUE), .groups = "drop")
          } else {
            .
          }
        },
...
# Fonction pour créer la carte de tension
creer_carte_tension <- function(map_data, npdc, tension_type = "Brute", annee) {
  ...
  couleurs_tension <- c(
    "≤1" = "#A8D38D", "1 à 4" = "#7BA05B", "5 à 7" = "#FFD990",
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("bassin",
        label = "Choix bassin :",
        choices = c(
          "Arras", "Béthune", "Boulogne-sur-Mer", "Calais", "Cambrai", "Dunkerque",
    ...
    mainPanel(
      navset_card_underline(
        nav_panel(
          ...
          p(
            tags$strong("Usage interne ", style = "font-size: 1.1em;"), br(), br(),
        ...
        nav_panel(
          ...
          tags$ul(
            ...
            tags$li(
              strong("Tension brute : "), "Indicateur de pression de la demande : nombre de demandes rapporté au nombre de Places proposées au tour principal.", br(), br(),
              em("Calcul : Demandes / Places proposées au tour")
            ), br(),
            tags$li(
              strong("Tension réajustée : "), "Indicateur prenant en compte la spécificité du dernier tour, en intégrant les places restées vacantes lors de la phase complémentaire.", br(), br(),
              ...
              em("** Places réservées = Places proposées au tour – Places non réservées par les étudiants")
            ), br(),
...
server <- function(input, output, session) {
  ...
  output$download_all_maps <- downloadHandler(
    ...
    content = function(file) {
      ...
      for (nom_bassin in names(bassins)) {
        ...
        map_data <- map_data %>%
          mutate(
            classe_brute = as.character(cut(`Tension Brute`,
              breaks = c(-Inf, 1, 5, 8, 10, Inf),
              labels = c("≤1", "1 à 4", "5 à 7", "8 à 9", ">10"), right = FALSE
            )),
            classe_pond = as.character(cut(`Tension réajustée`,
              breaks = c(-Inf, 1, 5, 8, 10, Inf),
              labels = c("≤1", "1 à 4", "5 à 7", "8 à 9", ">10"), right = FALSE
            )),
        ...
        pts <- bind_rows(
          map_data %>% mutate(
            Type = "Brute", Tension = `Tension Brute`,
        ...
        p <- ggplot() +
          geom_sf(data = npdc, fill = NA, color = "#004080", size = 0.4) +
          geom_point(
            data = pts, aes(x = Longitude, y = Latitude, fill = tension_classe, size = tension_classe),
            shape = 21, color = "white", stroke = 0.5
          ) +
          geom_label_repel(
            data = pts, aes(x = Longitude, y = Latitude, label = Label),
  ...
  output$download_tab <- downloadHandler(
    filename = function() {
      paste0("Tableau_tension_secteur_", input$select_annee, ".xlsx")
    },
    content = function(file) {
      req(data(), input$select_annee, input$bassin)
  ...
  output$download_tab1 <- downloadHandler(
    filename = function() {
      "Graphique_evolution_globale.png"
    },
    content = function(file) {
      ...
      p <- ggplot() +
        geom_col(
          data = d_long, aes(x = as.factor(`Année de gestion`), y = valeur, fill = type),
          position = position_dodge(width = 0.8), width = 0.7
        ) +
        geom_text(
          data = d_long, aes(x = as.factor(`Année de gestion`), y = valeur, label = valeur, group = type),
          position = position_dodge(width = 0.8), vjust = -0.5, size = 3
        ) +
        geom_line(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension réajustée` * max_y / max_tension, group = 1, color = "Tension réajustée"
        ), size = 1.2) +
        geom_point(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension réajustée` * max_y / max_tension, color = "Tension réajustée"
        ), size = 3) +
        geom_text(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension réajustée` * max_y / max_tension + 0.03 * max_y,
          label = round(`Tension réajustée`, 2)
        ), vjust = 4.5, size = 3, color = "black", fontface = "bold") +
        geom_line(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension brute` * max_y / max_tension, group = 1, color = "Tension brute"
        ), size = 1.2, linetype = "dashed") +
        geom_point(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension brute` * max_y / max_tension, color = "Tension brute"
        ), size = 3) +
        geom_text(data = resume_annee, aes(
          x = as.factor(`Année de gestion`),
          y = `Tension brute` * max_y / max_tension + 0.03 * max_y,
          label = round(`Tension brute`, 2)
        ), vjust = 0, size = 3, color = "black", fontface = "bold") +
        scale_fill_manual(values = c(
          "Étudiants demandeurs" = "#F4A280", "Étudiants demandeurs dernier tour" = "#F4A261",
          "Places proposées" = "#7EB3D5", "Places phase complémentaire" = "#7EB1E7"
        ), name = "") +
        scale_color_manual(values = c("Tension réajustée" = "#3498db", "Tension brute" = "#004080"), name = "") +
        scale_y_continuous(
          name = "Demandes et places", breaks = seq(0, max_y, by = 2000),
          labels = scales::label_number(scale = 1 / 1000, suffix = "k", accuracy = 1),
          sec.axis = sec_axis(~ . * max_tension / max_y,
  ...
  output$bubbleChart1 <- renderPlotly({
    ...
    p1 <- ggplot(tension_data1, aes(
      ...
      text = paste(
        "Année :", `Année de gestion`, "<br>Résidence :", Résidence,
      ...
      labs(
        x = "Nombre de demande", y = "Taux de tension", color = "Résidence", size = NULL,
  ...
  output$graph2 <- renderPlotly({
    ...
    plot_ly(resume_annee, x = ~ factor(`Année de gestion`)) %>%
      add_bars(
        y = ~`Étudiants demandeurs`, name = "Étudiants demandeurs", marker = list(color = "#F4A280"),
        text = ~`Étudiants demandeurs`, textposition = "outside", hoverinfo = "none"
      ) %>%
      add_bars(
        y = ~`Étudiants demandeurs dernier tour`, name = "Demandes dernier tour", marker = list(color = "#F4A261"),
        text = ~`Étudiants demandeurs dernier tour`, textposition = "outside", hoverinfo = "none"
      ) %>%
      add_bars(
        y = ~`Places proposées`, name = "Places proposées", marker = list(color = "#7EB3D5"),
        text = ~`Places proposées`, textposition = "outside", hoverinfo = "none"
      ) %>%
      add_bars(
        y = ~`Places phase complémentaire`, name = "Places phase complémentaire", marker = list(color = "#7EB1E7"),
      ...
      layout(
        ...
        yaxis = list(
          title = "Demandes et places", titlefont = list(size = 14), tickfont = list(size = 12),
          showgrid = TRUE, gridcolor = "rgba(200,200,200,0.3)"
        ),
        yaxis2 = list(
          title = "Tensions", overlaying = "y", side = "right", showgrid = FALSE,
  ...
  output$graph_boursiers <- renderPlotly({
    ...
    plot_ly(
      ...
      layout(
        ...
        annotations = list(
          list(
            text = paste0("Total<br>", sum(donut_data$`Étudiants demandeurs`)), x = 0.5, y = 0.5,
